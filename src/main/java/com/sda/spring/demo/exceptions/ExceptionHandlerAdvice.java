...
public class ExceptionHandlerAdvice {
    ...
    public ResponseEntity handleException(BookNotFoundException e) {
        return ResponseEntity.status(HttpStatus.NOT_FOUND)
                .body(new ApiError(HttpStatus.NOT_FOUND, "nie znaleziono rekordu", Arrays.asList("id","parameter"), LocalDateTime.now()));
    }

    @ExceptionHandler(CategoryNotFoundException.class)
    public ResponseEntity handleCategoryNotFoundException(CategoryNotFoundException e) {
        return ResponseEntity.status(HttpStatus.NOT_FOUND)
                .body(new ApiError(HttpStatus.NOT_FOUND, "nie znaleziono rekordu", Arrays.asList(""),LocalDateTime.now()));
    }
    ...
    public ResponseEntity inputHandleException(MethodArgumentNotValidException e){
        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                .body(new ValidationError(HttpStatus.BAD_REQUEST,e.getBindingResult().getAllErrors()));
    }
